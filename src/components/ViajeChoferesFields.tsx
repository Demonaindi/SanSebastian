...
export function ViajeChoferesFields({
  ...
  const addSlot = () => {
    ...
    onChange([...list, { chofer_id: '', viaticos: '', pago_viaje: '' }])
  }

  const removeSlot = (index: number) => {
    if (list.length <= 1) {
      onChange([{ chofer_id: '', viaticos: '', pago_viaje: '' }])
      return
    }
  ...
  return (
    <div className="space-y-3">
      ...
      {list.map((slot, index) => {
        ...
        return (
          <div key={index} className="space-y-2 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3">
            ...
            <FormField label="Pago de viaje">
              <input
                type="number"
                min="0"
                step="0.01"
                disabled={disabled || !slot.chofer_id}
                value={slot.pago_viaje}
                onChange={(e) => updateSlot(index, { pago_viaje: e.target.value })}
                className="input-field"
                placeholder="0"
              />
            </FormField>
